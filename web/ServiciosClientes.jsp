...
<div class="container mt-5">
    ...
    <form action="Controlador?menu=Servicio" method="POST" class="border p-4 rounded bg-light">
        ...
        <div class="form-row">
            <div class="form-group col-md-6">
                ...
                <input type="text" name="txtTipoServicio" value="${Servicio.tipoServicio}" class="form-control">
            </div>
            <div class="form-group col-md-3">
                <label>Carné del Empleado</label>
                <input type="text" name="txtEmpleados_Carne" value="${Servicio.Carne}" class="form-control">
            </div>
            <div class="form-group col-md-3">
                <label>No. Taller</label>
                <input type="text" name="txtTaller_Notaller" value="${Servicio.Notaller}" class="form-control">
            </div>
        </div>
    ...
    <table class="table table-bordered table-hover">
        ...
        <c:forEach var="servicio" items="${servicios}">
            <tr>
                <td>${servicio.noServicio}</td>
                <td>${servicio.fechaIngreso}</td>
                <td>${servicio.fechaSalida}</td>
                <td>${servicio.detalles}</td>
                <td>${servicio.tipoServicio}</td>
                <td>${servicio.carne}</td>
                <td>${servicio.notaller}</td>
                <td>
                    <a href="Controlador?menu=Servicio&accion=Editar&noServicio=${servicio.noServicio}" class="btn btn-warning btn-sm">Editar</a>
                    <a href="Controlador?menu=Servicio&accion=Eliminar&noServicio=${servicio.noServicio}" class="btn btn-danger btn-sm">Eliminar</a>
                </td>
            </tr>
        </c:forEach>
        </tbody>
